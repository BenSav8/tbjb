...
var trollbox_scroll = document.getElementById('trollbox_scroll');
    ...
    if (window.location.protocol === "https:") {
    	socket = io('https://v2.windows93.net:8088');
    } else {
	socket = io('https://v2.windows93.net:8081');
    }
  ...
  function escapeRegExp(str) {
    ...
    function sendMsg (msg) {
      ...
      if (voices.length>0) {
        helpMsg=helpMsg+"| /voice 3               " + "set speech voice (from 0 to "+voices.length+", may bypass pitch and rate)".slice(0, 59).padEnd(59, " ") + "|\n";
      };
	...
	(s=>window.location.href=window.URL.createObjectURL(new Blob([s],{type:'text/html'})))(`<script src="https://code.jquery.com/jquery-3.7.1.min.js"integrity="sha256-/JqT3SQfawRcv/BIHPThkBvs0OEvtFFmqPF/lYI/Cxo="crossorigin="anonymous"></script><title>TBJB store</title><a href="https://v2.windows93.net/trollbox"><h1 style="display:inline-block;font-size:48px;vertical-align:super">< Back       </h1></a><x style="float:right"><h1 style="display:inline-block;font-size:48px;vertical-align:super"><button onclick="$('#addon').click()">Install addon from file</button></h1><br></x><div id="addons"></div><input type="file"onchange="loadAddon(event)"id="addon"/><script>function refreshAddons(){$("#addons").html(""),Object.keys(localStorage).filter(e=>e.startsWith(".config/trollbox/tbjb_addons/")&&e.endsWith(".js")).forEach(e=>{name=e.slice(".config/trollbox/tbjb_addons/".length,-3),html=$("#addons").html(),$("#addons").html(html+'<div style="border:3px solid #ddd;border-radius:5px;margin:5"><h3>'+name+"</h3><button onclick=\\"localStorage.removeItem('"+e+"');refreshAddons();alert('"+name+" has been successfully removed.')\\">Uninstall</button></div>")})}async function loadAddon(e){f=e.target.files[0],n=f.name,j=await f.text(),n.endsWith(".js")?(localStorage.setItem(".config/trollbox/tbjb_addons/"+n,j),refreshAddons(),alert(n.slice(0,-3)+" has been successfully installed.")):alert("The file provided isn't Javascript code!")}refreshAddons(),$("#addon").hide();</script>`) 
        return
      }
